...
namespace Competitions.Web.Areas.Managment.Controllers
{
    ...
    public class ExtracurricularController : Controller
    {
        ...
        public async Task<IActionResult> Create ()
        {
            ...
            {
                Places = await _placeRepo.GetAllAsync<SelectListItem>(u => u.ParentPlaceId == null , select: u => new SelectListItem
                {
                    Text = u.Title ,
                    Value = u.Id.ToString()
                }) ,

                AudienceTypes = await _audRepo.GetAllAsync<SelectListItem>(select: u => new SelectListItem { Text = u.Title , Value = u.Id.ToString() })
            };
        ...
        public async Task<IActionResult> Create ( CreateExtracurricularDto command )
        {
            ...
            {
                command.Places = await _placeRepo.GetAllAsync<SelectListItem>(u => u.ParentPlaceId == null ,
        ...
        public async Task<IActionResult> Update ( Guid id )
        {
            ...
            dto.Places = await _placeRepo.GetAllAsync<SelectListItem>(u => u.ParentPlaceId == null , select: u => new SelectListItem
            { Text = u.Title , Value = u.Id.ToString() });
            dto.AudienceTypes = await _audRepo.GetAllAsync<SelectListItem>(select: u => new SelectListItem { Text = u.Title , Value = u.Id.ToString() });
            return View(dto);
        }
        [HttpPost]
        ...
        {
            var subPlaces = await _placeRepo.GetAllAsync<SelectListItem>(u => u.ParentPlaceId == id , select: u => new SelectListItem
            {
                Text = u.Title ,
                Value = u.Id.ToString()
            });
